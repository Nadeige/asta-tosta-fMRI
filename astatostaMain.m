...
for j=1:nrRuns
    ...
    for i=1:nrTrials
            ...
            case {'mriScanner', 'mriSimulator'}
                ...
                while ~validated % wait for press
                    if (SerPort.BytesAvailable)
                        ...
                        k = k+1;
                        secs(k) = GetSecs;
                        [Events, nbevents] = LogEvents(Events, nbevents,  'Button Press', sbutton, secs(k));
                        if sbutton == 5
                            keyName{k} = sbutton;
                            sbutton = 0;
                        elseif sbutton == 1
        ...
        time.end = GetSecs;
        [Events, nbevents] = LogEvents(Events, nbevents,  'Picture', 'DispChoiceSelect', time, Jittime, info_choice);
        [Events, nbevents] = LogEvents(Events, nbevents,  'Button Press', KeyNames, Keytimes); 
    ...
    switch Cfg.run_mode
        case {'mriScanner', 'mriSimulator_serial'}
            Beeper
            if runnb(trialnb,1) < nrRuns
                DisplayInstructionFile_noresp(fullfile('Instrfiles', 'instr_end_block_MRI.txt'), 22, win); % display end of session message
            else
                DisplayInstructionFile_noresp(fullfile('Instrfiles', 'instr_end.txt'), 22, win);
            end
            time.start = GetSecs;
            time.end=NaN;
            [Events, nbevents] = LogEvents(Events, nbevents,  'Picture', 'End of session', time, NaN);
            % wait for scanner to stop
            pulsetime = Wait_ScannerStops(SerPort, Cfg.scannerSynchTimeOutMs, Cfg.TR);
            [Events, nbevents] = LogEvents(Events, nbevents,  'Pulse', [], pulsetime);
            % Close serial port
            CloseSerialPort(SerPort);
